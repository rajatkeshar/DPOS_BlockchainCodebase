...
function verifyGenesisBlock(scope, block) {
  try {
    ...
    var id = scope.base.block.getId2(block);
...
function main() {
  ...
  program
    ...
    .option('-d, --daemon', 'Run blockchain node as daemon')
  ...
  var pidFile = path.join(baseDir, 'chain.pid');
  if (fs.existsSync(pidFile)) {
    console.log('Failed: Blockchain server already started');
    return;
  }
  ...
  if (program.daemon) {
    console.log('Blockchain server started as daemon ...');
