...
logfile=$root_path/logs/blockchain_manager.log
ipfs_log=$root_path/logs/ipfs.log


readonly PROG_DIR=$(pwd)
readonly PID_FILE=$PROG_DIR/chain.pid
...
function install_prereq() {
	...
	echo ""
	echo "-------------------------------------------------------"
	echo "Blockchain installer script. Version: $version"
	echo "-------------------------------------------------------"
...
function install_blockchain() {
    ...
    echo -n "Installing Blockchain core... "
    npm install --production &>> $logfile || { echo "Could not install Blockchain, please check the log directory. Exiting." && exit 1; }
    #npm install nan@2.2.1 &>> $logfile || { echo "Could not install Blockchain, please check the log directory. Exiting." && exit 1; }
    npm install npm install sodium --unsafe-perm &>> $logfile || { echo "Could not install Blockchain, please check the log directory. Exiting." && exit 1; }
    echo -e "done.\n"
...
function status() {
  if is_running; then
    ...
    echo "Blockchain server is not running"
  fi
}

function start() {
  if is_running; then
    echo "Blockchain server is already started"
  else
    rm -f $PROG_DIR/chain.pid
...
function stop() {
  ...
  if [ -n "$pid" ] && ps -p "$pid" > /dev/null; then
    ...
    while ps -p $pid > /dev/null; do
      if [ $i == 5 ]; then
        kill -9 $pid
        echo "Blockchain server killed"
      fi
      echo "Still waiting for Blockchain server to stop ..."
      sleep 1
      ((i++))
    done
    echo "Blockchain server stopped"
  else
    echo "Blockchain server is not running"
  fi
  rm -f $PID_FILE
