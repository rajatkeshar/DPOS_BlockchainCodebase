...
angular.module('liskApp').controller('sendTransactionController', ['$scope', 'sendTransactionModal', '$http', 'userService', 'feeService', '$timeout', '$filter', function ($scope, sendTransactionModal, $http, userService, feeService, $timeout, $filter) {
    ...
    $scope.isCorrectValue = function (currency, throwError) {
        ...
        if (amount == '') {
            return error('Belrium amount can not be blank');
        }
        ...
        if (parts.length == 1) {
            ...
            if (parts[1].length > 8) {
                return error('Belrium amount must not have more than 8 decimal places');
            ...
            return error('Belrium amount must have only one decimal point');
        }
        ...
        if (amount == '0' && fraction == '00000000') {
            return error('Belrium amount can not be zero');
        }
        ...
        if (!/^\d+$/.test(result)) {
            return error('Belrium amount contains non-numeric characters');
        }
