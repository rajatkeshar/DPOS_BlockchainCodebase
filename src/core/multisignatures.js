...
var RequestSanitizer = require('../utils/request-sanitizer.js');
...
function Multisignature() {
  ...
  this.calculateFee = function (trs, sender) {
    return ((trs.asset.multisignature.keysgroup.length + 1) * constants.fees.multisignature) * constants.fixedPoint;
  }
          ...
          if (trs.signatures) {
            for (var d = 0; d < trs.signatures.length && !verify; d++) {
              if (trs.asset.multisignature.keysgroup[s][0] != '-' && trs.asset.multisignature.keysgroup[s][0] != '+') {
                ...
                verify = library.base.transaction.verifySignature(trs, trs.asset.multisignature.keysgroup[s].substring(1), trs.signatures[d]);
              }
            }
          }
  ...
  if (transaction.type == TransactionTypes.MULTI) {
    ...
    try {
      for (var i = 0; i < transaction.asset.multisignature.keysgroup.length && !verify; i++) {
        ...
        verify = library.base.transaction.verifySignature(transaction, key, tx.signature);
      }
    } catch (e) {
    ...
    modules.accounts.getAccount({
      ...
      try {
        for (var i = 0; i < multisignatures.length && !verify; i++) {
          verify = library.base.transaction.verifySignature(transaction, multisignatures[i], tx.signature);
        }
      } catch (e) {
        return cb("Failed to verify signature");
      }
